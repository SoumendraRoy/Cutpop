...
mdeto = np.random.normal(mdet, 4.0)
...
mcmc.run(jax.random.PRNGKey(np.random.randint(1<<32)), ms_det[np.median(ms_det, axis=1)>50.0, :], zs_det[np.median(ms_det, axis=1)>50.0, :], mdet[mdeto>50.0], zdet[mdeto>50.0])
samples = mcmc.get_samples()
with h5py.File('hyperparameter_higher_mass_peak_w_norm.h5','w') as hf:
